...
def index():
    ...
    targettime =  datetime(basetime.year, basetime.month, basetime.day, basetime.hour, 00, 00, 0000)
    for i in citylist:
        if loopCnt == 0:
            try:
                #tzlist.append(BuildTimezoneList(datetime.now(pytz.timezone(tzdict[i])),i))
                #tzlist.append(BuildTimezoneList(targettime.now(pytz.timezone(tzdict[i])),i))
                timezoneinfo=pytz.timezone(tzdict[i])
                tzlist.append(BuildTimezoneList(targettime.astimezone(timezoneinfo),i))
            except:
                resp = make_response(redirect('/'))
                resp.set_cookie('cities', 'Tokyo,Singapore,IST,New York')
                return resp
            ...
            try:
                ...
                # tzlist.append(BuildTimezoneListnoYear(targettime.now(pytz.timezone(tzdict[i])),i))
                timezoneinfo=pytz.timezone(tzdict[i])
                tzlist.append(BuildTimezoneListnoYear(targettime.astimezone(timezoneinfo),i))
            except:
                resp = make_response(redirect('/'))
                resp.set_cookie('cities', 'Tokyo,Singapore,IST,New York')
                return resp
